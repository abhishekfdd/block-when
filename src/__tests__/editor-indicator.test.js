 ...
 * `has-renderwhen-rule` to its `className` when the block has an
...
describe( 'withRuleIndicator', () => {
	...
	it( 'leaves className unchanged when renderWhen attribute is absent', () => {
	...
	it( 'adds has-renderwhen-rule when renderWhen has a registered conditionId', () => {
		...
		const html = render(
			{ renderWhen: { conditionId: 'user_state', settings: {} } },
			{ className: 'wp-block' }
		);

		expect( html ).toBe(
			'<div data-class="wp-block has-renderwhen-rule"></div>'
		);
	...
	it( 'leaves className unchanged when conditionId is not registered', () => {
		...
		const html = render(
			{ renderWhen: { conditionId: 'orphaned_id', settings: {} } },
			{ className: 'wp-block' }
		);
	...
	it( 'leaves className unchanged when renderWhen has no conditionId field', () => {
		const html = render(
			{ renderWhen: { settings: {} } },
			{ className: 'wp-block' }
		);
